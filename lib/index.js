...
var redirectToService = function(callbackUrl, cb) {
  ...
  oauth.getOAuthRequestToken(function(err, token, tokenSecret){
    cb(err, authorizeURL + "?oauth_token=" + token + "&name=" + encodeURIComponent(config.trelloAppname), {token: token, tokenSecret: tokenSecret, callbackUrl: callbackUrl});
  });
};
...
var retrieveTokens = function(reqParams, storedParams, cb) {
  // Store new data
  //
  var oauth = new OAuth(requestURL, accessURL, config.trelloKey, config.trelloSecret, "1.0", storedParams.callbackUrl, "HMAC-SHA1");
  var oauth_token = reqParams.oauth_token;
  var oauth_verifier = reqParams.oauth_verifier;
  var tokenSecret = storedParams.tokenSecret;
  console.log("inside retrieveTokens");
  oauth.getOAuthAccessToken(oauth_token, tokenSecret, oauth_verifier, function(err, accessToken, accessTokenSecret){
    cb(err, 'accountName', {accessToken: accessToken, accessTokenSecret: accessTokenSecret, callbackUrl: storedParams.callbackUrl });
  });
