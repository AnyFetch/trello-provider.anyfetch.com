...
var cleanUp = function(rawCard) {
  ...
  card.comments = rawCard.actions
    ...
    .map(function(comment) {
      return {
        text: entities.decode(comment.data.text),
        creator: entities.decode(comment.memberCreator.fullName)
      };
    });
  ...
  card.labels = rawCard.labels.map(function(label) {
    return {
      name: entities.decode(label.name),
      color: label.color
    };
  });

  card.board = entities.decode(rawCard.board);

  card.list = entities.decode(rawCard.list);
  ...
  card.title = entities.decode(rawCard.name);

  card.description = entities.decode(rawCard.desc);
