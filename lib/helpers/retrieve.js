  ...
  card.comments = rawCard.actions
    ...
    .map(function(comment) {
      return {
        ...
        creator: comment.memberCreator ? (comment.memberCreator.fullName ? entities.decode(comment.memberCreator.fullName) : "") : ""
      };
    });
...
var queryWithRetry = function queryWithRetry(t, verb, endpoint, params, retries, cb) {
  if(!cb) {
    cb = retries;
    retries = 0;
  }
  t[verb](endpoint, params, function(err, res) {
    if(err && err.statusCode === 429 && retries <= MAX_RETRY) {
      retries += 1;
      setTimeout(function retry() {
        queryWithRetry(t, verb, endpoint, params, retries, cb);
      }, (res.headers.get('Retry-After') || 10) * 1000); // if no header, wait 10 seconds
    }
    else {
      cb(err, res);
    }
...
var retrieveBoards = function(accessToken, cb) {
  ...
  queryWithRetry(t, 'get', "1/members/my/boards", {lists: 'all'}, function(err, boards) {
    if(err) {
      return cb(err);
    }
    var boardIds = [];
...
var retrieveCards = function(accessToken, since, cb) {
  ...
  async.waterfall([
    ...
    function getCards(boardsName, listsName, boards, cb) {
      ...
      queryWithRetry(t, 'get', "/1/batch", {urls: urls}, rarity.carry([boardsName, listsName], cb));
    },
  ...
  async.waterfall([
    ...
    function getCardsFromLists(cardsToDelete, listsToDelete, cb) {
      if(listsToDelete.length > 0) {
        ...
        queryWithRetry(t, 'get', "/1/batch", {urls: urls}, rarity.carry([cardsToDelete], cb));
      }
      else {
